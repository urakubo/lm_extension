function [inode, ihedra, iface] = saveabaqus_mod2(node,face,elem,fname,heading)
%
...
%%
%% H.U modified this file on 170614. 
%%
%% 1. S3R => STRI3
%% 2. Return the nodes, tetrahedra, and triangles with their indices, as [inode, ihedra, iface]
%%
...
if(~isempty(elem))
  ...
  for i=1:len
	...
	%%%
	  tmp = [count+(1:length(idx))' elem(idx,1:4)];
      ihedra(i) = {tmp};
    %%%
...
if(~isempty(face))
  ...
  for i=1:len
      ...
      fprintf(fid,'*ELEMENT, ELSET=MeshTri%d, TYPE=STRI3\n',labels(i));
      fprintf(fid,'%d,\t%d,\t%d,\t%d\n',[count+(1:length(idx))' face(idx,1:3)]');
	%%%
	  tmp = [count+(1:length(idx))' face(idx,1:3)];
      iface(i) = {tmp};
    %%%
      count=count+length(idx);
      if(length(elsetall)==0)
