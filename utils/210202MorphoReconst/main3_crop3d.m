...
FILENAME = sprintf('%s%svoxels_rotated_cropped.mat', p.OutputDir, p.F);
save(FILENAME,'bw_Dend_crop','bw_Mito_crop','bw_PSD_crop','bw_ER_crop');

bw_Dend_and_PSD = (bw_Dend_crop & bw_PSD_crop);
bw_Dend_not_PSD = (bw_Dend_crop & not(bw_PSD_crop));

hdf5write('CA1_2.h5','/dendrite',uint8(bw_Dend_crop),'/PSD',uint8(bw_PSD_crop),'/Mitochondrion',uint8(bw_Mito_crop),'/ER',uint8(bw_ER_crop),'/dendrite_and_PSD', uint8(bw_Dend_and_PSD),'/dendrite_not_PSD', uint8(bw_Dend_not_PSD));
